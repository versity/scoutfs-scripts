...
WR_RESULTS="/tmp/versity.fio.wr.${TEST_DATE}.txt"
RD_RESULTS="/tmp/versity.fio.rd.${TEST_DATE}.txt"
...
case "$RW" in
	...
	read)
		...
		;;
	fdx)
		fio "${BASE_FIO_OPTIONS[@]}" --name="${RD_JOB_NAME}" --rw=write --size="${SIZE}" --directory="${BENCH_DIR}" > /dev/null 2>&1
